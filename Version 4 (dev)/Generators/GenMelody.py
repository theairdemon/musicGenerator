...
class GenMelody:
    ...
    def genNote(self, measure, measure_idx, idx, progression):
        ...
        note_choices = {n: self.scale[(scale_idx + n) % len(self.scale)] for n in note_indices}
        ...
        for key in list(self.melody_info.note_weights.keys()):
            ...
            if r_note <= current_weights:
                random_choices = []
                if progression >= 0:
                    random_choices.append(key)
                if progression <= 0:
                    random_choices.append(-1 * key)
    ...
    def adjustMeasure(self, previous_idx, idx, ratio):
        ...
        if r_inverted <= self.melody_info.measure_adjustments['inverted']:
            working_measure = [[working_measure[-1 * (i+1)][0], working_measure[i][1]] for i in range(len(working_measure))]
    ...
    def slightBasedMeasure(self, previous_idx, idx, measure):
        ...
        for note in measure:
            ...
            # this guarntees the first note is always in our chord
            if note[0] in previous_chord_notes and note[0] not in chord_notes:
                previous_note_idx = self.scale.index(note[0])
                chord_note_differences = [self.scale.index(elem) - previous_note_idx for elem in chord_notes]
                closest_note_diff = min(chord_note_differences, key=abs)
                closest_note_idx = (previous_note_idx + closest_note_diff) % len(self.scale)
    ...
    def chordBasedMeasure(self, previous_idx, idx, measure):
        ...
        note_difference = self.scale.index(starting_note) - self.scale.index(previous_starting_note)
